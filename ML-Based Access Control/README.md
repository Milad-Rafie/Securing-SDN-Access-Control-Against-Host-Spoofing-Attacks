...
* **`DDoS_Mininet.ipynb`**: The Jupyter Notebook used to teach the system what an attack looks like. [cite_start]It processes data and trains the model[cite: 3, 11].
...
* **`Model/`**: This folder contains the pre-trained "brain" files generated by the notebook. [cite_start]You need these for the controller to work[cite: 21].
...
1.  **Training (The Notebook):** We used the InSDN dataset to train an XGBoost model. [cite_start]We taught it to ignore specific IP addresses and focus on behaviors like packet timing, size, and flag counts[cite: 7, 18].
...
3.  [cite_start]The last step of the notebook will generate new `.pkl` files for the `Model/` folder[cite: 21].

## Features Detected
[cite_start]To keep the system fast, we only look at the **Top 12** most important behaviors[cite: 18], including:
* Average Packet Size
